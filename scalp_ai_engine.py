...
class ScalpSignalEngine:
    ...
    def __init__(self):
        ...
        # Confidence thresholds for scalp signals
        self.min_scalp_confidence = 0.85  # 85% minimum
        self.premium_scalp_confidence = 0.92  # 92%+ for premium signals
        
        # Supported symbols for scalping (LIMITED FOR FAST SCANNING)
        # Only high-liquidity instruments: Gold, Silver, Bitcoin
        self.scalp_symbols = {
            'metals': ['XAUUSD', 'XAUEUR', 'XAGUSD'],
            'crypto': ['BTCUSD'],
        }
        
        # TradingView symbol mappings - using OANDA/FXCM for reliable prices
        self.tv_mappings = {
            'XAUUSD': ('OANDA', 'XAUUSD', 'cfd'),  # OANDA gold
            'XAUEUR': ('OANDA', 'XAUEUR', 'cfd'),  # OANDA gold/EUR
            'XAGUSD': ('OANDA', 'XAGUSD', 'cfd'),  # OANDA silver
            'BTCUSD': ('BITSTAMP', 'BTCUSD', 'crypto'),  # Bitstamp BTC
        }
        
        # Price validation ranges (reasonable market prices - UPDATE REGULARLY!)
        # These ranges are generous to allow for market volatility
        self.price_ranges = {
            'XAUUSD': (2500, 6000),   # Gold: wide range for volatility
            'XAUEUR': (2300, 5500),   # Gold/EUR: wide range
            'XAGUSD': (25, 120),      # Silver: $25-$120 (futures may be higher)
            'BTCUSD': (50000, 200000), # Bitcoin: wide range for crypto volatility
        }
    ...
    def _update_live_prices(self, force: bool = False):
        ...
        fallback_prices = {
            'XAUUSD': 4750.0,   # Gold futures ~$4750 (Jan 2026)
            'XAUEUR': 4570.0,   # Gold/EUR ~€4570
            'XAGUSD': 94.0,     # Silver futures ~$94
            'BTCUSD': 90000.0,  # Bitcoin ~$90,000
        }
        
        fetched_any = False
        logger.info("📊 Fetching REAL-TIME prices...")
        
        # ========== METHOD 1: yfinance (PRIMARY - MOST RELIABLE) ==========
        ...
        # ========== METHOD 2: CoinGecko for crypto (backup) ==========
        ...
        # ========== METHOD 3: TradingView (backup - often rate limited) ==========
        if TRADINGVIEW_AVAILABLE:
            for symbol, mapping in self.tv_mappings.items():
    ...
    def _get_fallback_price(self, symbol: str) -> float:
        """Get fallback price for symbol (approximate Jan 2026 prices)"""
        fallback_prices = {
            'XAUUSD': 2680.0,   # Gold ~$2680
            'XAUEUR': 2580.0,   # Gold/EUR ~€2580
            'XAGUSD': 30.80,    # Silver ~$30.80
