...
class AdvancedMarketAnalyzer:
    ...
    def __init__(self):
        ...
        self.base_prices = {
            ...
            'XAUUSD': 4750.0, 'XAGUSD': 94.0,  # Updated Jan 2026
            'USOIL': 77.20, 'UKOIL': 80.50,
            'US30': 44200.0, 'US500': 6050.0, 'US100': 21800.0,
            'GER40': 21000.0, 'UK100': 8500.0, 'JPN225': 38900.0,
            'BTCUSD': 90000.0, 'ETHUSD': 3200.0, 'BNBUSD': 700.0,  # BTC ~$90k Jan 2026
            'XRPUSD': 3.10, 'SOLUSD': 250.0,
        }
        ...
        # Fetch real-time prices on initialization
        self._update_live_prices()
    
    def _update_live_prices(self):
        """Fetch REAL-TIME prices from yfinance"""
        if not YFINANCE_AVAILABLE:
            logger.warning("yfinance not available - using fallback prices")
            return
        
        logger.info("📊 Fetching REAL-TIME prices for autonomous generator...")
        
        for our_symbol, yf_symbol in self.yf_symbols.items():
            try:
                ...
                if price and price > 0:
                    ...
                    logger.info(f"📊 {our_symbol}: ${price:,.2f}")
            except Exception as e:
                logger.debug(f"Could not fetch {our_symbol}: {e}")
...
class AutonomousSignalGenerator:
    ...
    def __init__(self, telegram_token: str = None, chat_ids: List[int] = None):
        ...
        self.min_confidence = 0.75  # Minimum for regular signals
        self.scalp_confidence = 0.90  # Minimum for scalp signals (very high)
